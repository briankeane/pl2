...
describe "MoveSpin" do
  ...
  before(:each) do
    ...
    @spin1 = PL.db.create_spin({ station_id: @station.id, audio_block_id: @song1.id, current_position: 5 })
    @spin2 = PL.db.create_spin({ station_id: @station.id, audio_block_id: @song2.id, current_position: 6 })
    @spin3 = PL.db.create_spin({ station_id: @station.id, audio_block_id: @song3.id, current_position: 7 })
    @spin4 = PL.db.create_spin({ station_id: @station.id, audio_block_id: @song4.id, current_position: 8 })
    @spin5 = PL.db.create_spin({ station_id: @station.id, audio_block_id: @song5.id, current_position: 9 })
    @spin6 = PL.db.create_spin({ station_id: @station.id, audio_block_id: @song6.id, current_position: 10 })
    @station.last_accurate_current_position = 10
  end

  it "calls bullshit if the station_id is invalid" do
    result = PL::MoveSpin.run({ station_id: 999,
                                  old_position: 8,
                                  new_position: 7 })
  ...
  it "calls bullshit if old_position is invalid" do
    ...
    result = PL::MoveSpin.run({ station_id: @station.id,
                                  old_position: 999,
                                  new_position: 7 })
    expect(result.success?).to eq(false)
  ...
  it "moves a spin" do
    ...
    expect(PL.db.get_full_playlist(@station.id).map { |spin| spin.audio_block.id }).to eq([@song1.id, @song2.id, @song4.id,@song5.id,@song3.id,@song6.id])
  end
  ...
  it 'rearranges the last_accurate_current_position after moving the spin' do
    ...
    result = PL::MoveSpin.run({ station_id: @station.id,
                                  ...
                                  new_position: 9 })
    expect(result.success?).to eq(true)
    expect(@station.last_accurate_current_position).to eq(6)
  end
