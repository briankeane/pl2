...
module PL
  class ProcessSong < UseCase
    def run(attrs)
      ...
      # add extension to end of temp name
      if File.extname(temp_song_file) != extension
        temp_song_file.path = temp_song_file.path + extension
      end
      ...
      case 
      when (extension == '.mp4') || (extension == '.m4a') || (extension == '.m4p')
        temp_song_file = File.open(ac.mp4_to_mp3(temp_song_file))
      end
